...
| TC-AUTO-06 | Integracion | Crear cuenta, movimiento, retiro invalido y reporte | Respuestas HTTP esperadas y reporte con saldo correcto |
...
| TC-KARATE-04 | `cuentas-movimientos.feature` | Crear cuenta, deposito, reporte y retiro sin saldo | HTTP 201/200/400 segun cada paso |
...
| TC-MAN-06 | Account | Registrar retiro de -300 | HTTP 400 y mensaje `Saldo no disponible` |
| TC-MAN-07 | Account | Consultar `/reportes?fecha=2020-01-01,2030-12-31&cliente=<clienteId>` | Retorna movimientos del cliente |
| TC-MAN-08 | Docker | Levantar `docker compose up --build` desde WSL | Servicios, PostgreSQL y RabbitMQ disponibles |
